<template>
  <b-navbar toggleable="lg" type="dark" class="nav-color">
    <b-container>
      <b-navbar-brand href="#">
        <svg
          version="1.1"
          id="Layer_1"
          width="163px"
          height="36.44px"
          xmlns="http://www.w3.org/2000/svg"
          xmlns:xlink="http://www.w3.org/1999/xlink"
          x="0px"
          y="0px"
          viewBox="0 0 81.4 18.2"
          style="enable-background:new 0 0 81.4 18.2;"
          xml:space="preserve"
        >
          <g>
            <g>
              <path fill="#0169D0" d="M12.2,3.5h-1.4v10.4h2.6V4.7C13.5,3.9,13.1,3.5,12.2,3.5z" />
              <path fill="#0169D0" d="M12.2,0.5h-1.4v2.1h2.6V1.8C13.5,0.9,13.1,0.5,12.2,0.5z" />
              <path fill="#0169D0" d="M15.9,0.5h-1.4v13.4h2.6V1.8C17.2,1.1,16.6,0.5,15.9,0.5z" />
              <path
                fill="#0169D0"
                d="M23,3.3c-3.2,0-4.8,1.8-4.8,5.4s1.6,5.4,4.7,5.5c2.6,0,4-1.2,4.6-3.4h-1.9c-0.4,0-0.7,0.1-0.9,0.4
			C24.2,11.7,23.6,12,23,12c-1.3,0-2-0.8-2.1-2.4h6.8C27.9,5.4,26.4,3.3,23,3.3z M21,7.6c0.2-1.4,0.9-2.1,2-2.1s1.8,0.7,1.9,2.1H21z
			"
              />
              <path
                fill="#0169D0"
                d="M31.7,5.4c0.7,0,1.3-0.6,1.3-1.3V3.5h-1.6V1.8c0-0.7-0.6-1.3-1.3-1.3h-1.4v11.4c0,1.2,0.5,1.8,1.4,1.9
			c1,0.1,1.9,0.1,2.8,0v-2h-0.8c-0.5,0-0.8-0.2-0.8-0.7V5.3L31.7,5.4L31.7,5.4z"
              />
              <path
                fill="#0169D0"
                d="M5.6,3.3c-1.3,0-2.3,0.5-3,1.6V1.8c0-0.9-0.4-1.3-1.3-1.3H0v8.9c0,0.1,0,0.3,0,0.4v0.1v3.8
			c0,0.2,0.2,0.4,0.4,0.4h1.8c0.2,0,0.4-0.1,0.4-0.3C3.2,14,4,14.1,4.9,14.1c3.2-0.1,4.9-1.8,4.9-5.3C9.8,5.2,8.4,3.4,5.6,3.3z
			 M4.9,11.8c-1.5,0-2.2-1.1-2.2-3.2c0.1-2,0.8-3,2.2-3s2.2,1,2.2,3C7.1,10.8,6.4,11.8,4.9,11.8z"
              />
            </g>
            <g>
              <path
                fill="#7cb900"
                d="M39.9,3c-1.3,0-2.4,0.5-3.1,1.7V1.5c0-0.9-0.4-1.4-1.3-1.4h-1.4v9.2c0,0.1,0,0.3,0,0.4v0.1v3.9
			c0,0.2,0.2,0.4,0.4,0.4h1.9c0.2,0,0.4-0.1,0.4-0.3c0.7,0.2,1.4,0.3,2.4,0.3c3.3-0.1,5-1.9,5-5.5C44.2,4.9,42.7,3.1,39.9,3z
			 M39.1,11.8c-1.5,0-2.3-1.1-2.3-3.3c0.1-2.1,0.8-3.1,2.3-3.1s2.2,1,2.3,3.1C41.4,10.7,40.6,11.8,39.1,11.8z"
              />
              <path
                fill="#7cb900"
                d="M49.4,3c-2.5,0-3.9,1-4.2,3h2.7c0.1-0.5,0.7-0.8,1.6-0.8c1.1,0,1.7,0.3,1.8,0.9c0,0.6-0.6,0.9-1.8,1.1
			c-2.7,0.3-4.6,1-4.7,3.5c0,2.4,1.8,3.6,4.4,3.4c0.7-0.1,1.4-0.2,2-0.4c0,0.2,0.2,0.3,0.4,0.3h1.9c0.2,0,0.4-0.2,0.4-0.4V9.7l0,0
			V9.6V5.9C53.9,4,52.4,3,49.4,3z M48.9,11.9c-0.9,0-1.3-0.4-1.4-1.3c0-0.8,0.5-1.3,1.6-1.5c0.8-0.2,1.5-0.4,2.2-0.7v1
			C51.3,11,50.4,11.9,48.9,11.9z"
              />
              <path
                fill="#7cb900"
                d="M61.1,4.1l-1.9,6.5l-2.1-7.5H54l3.6,10.6c0.4,1.4-0.1,2.2-1.6,2.2h-0.8v2.2c0.1,0.1,0.4,0.1,1.1,0.1
			c2.2,0,2.9-0.6,3.5-2.4l4.3-12.6h-1.9C61.7,3.1,61.2,3.5,61.1,4.1z"
              />
              <path fill="#7cb900" d="M66.3,0h-1.4v2.1h2.7V1.3C67.7,0.5,67.2,0,66.3,0z" />
              <path fill="#7cb900" d="M66.3,3.1h-1.4v10.8h2.7V4.4C67.6,3.6,67.2,3.1,66.3,3.1z" />
              <path
                fill="#7cb900"
                d="M73.6,7.2c-1.4-0.3-2.1-0.7-2.1-1.2s0.5-0.8,1.6-0.8c0.9,0,1.4,0.3,1.6,0.7s0.6,0.7,1.2,0.7h1.7
			c-0.1-1.1-0.5-2-1.3-2.6S74.4,3.1,73,3.1c-2.5,0-4.3,1.1-4.3,3.5c0,1.7,1.2,2.8,3.6,3.2c1.9,0.3,2.8,0.8,2.8,1.3
			c0,0.7-0.6,1-1.7,1c-1.3,0-2.2-0.4-2.2-1.6h-2.8c0,1.2,0.4,2.2,1.2,2.8c0.7,0.6,1.9,1,3.4,1c2.9,0,4.9-1,4.9-3.6
			c0-0.7-0.2-1.4-0.8-1.9C76.6,8,75.4,7.5,73.6,7.2z"
              />
              <path fill="#7cb900" d="M80.1,0h-1.4v2.1h2.7V1.3C81.5,0.5,81,0,80.1,0z" />
              <path fill="#7cb900" d="M80.1,3.1h-1.4v10.8h2.7V4.4C81.4,3.6,81,3.1,80.1,3.1z" />
            </g>
          </g>
        </svg>
      </b-navbar-brand>

      <b-navbar-toggle target="nav-collapse"></b-navbar-toggle>

      <b-collapse id="nav-collapse" is-nav>
        <!-- Right aligned nav items -->
        <b-navbar-nav class="ml-auto">
          <b-nav-form>
            <b-button-group>
              <b-button class="mr-3" variant="outline-info">Kayıt Ol</b-button>
              <b-button variant="outline-info">Giriş</b-button>
            </b-button-group>
          </b-nav-form>
        </b-navbar-nav>
      </b-collapse>
    </b-container>
  </b-navbar>
</template>

<script>
export default {
  name: "Navbar"
};
</script>

<style>
.nav-color {
  color: khaki;
}
</style>